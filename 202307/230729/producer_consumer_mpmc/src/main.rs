...
fn main() {
    ...
    let lock_clone_3 = Arc::clone(&lock);
    let queue = Arc::new(Mutex::new(VecDeque::with_capacity(10)));
    ...
    let consumer_1 = thread::spawn(move || {
        loop {
            let mut lock_consumer = lock_clone_3.lock().unwrap();
            let mut queue_consumer = queue_clone_3.lock().unwrap();
