...
impl<T> ArcImmut<T> {
    ...
    pub fn back_to_mut(this: Self) -> ArcMut<T> {
        ...
        let mut new_this = ManuallyDrop::new(this);
        let inner = unsafe { ptr::read(Self::get_mut_unchecked(&mut new_this))};
        ArcMut { data: inner }
    }
}
...
fn main() {
    ...
    unsafe {
        ...
        let builder = thread::Builder::new();
        let t1 = builder.spawn_unchecked(|| {
            println!("{}", a2);//0.5でprint
        });
        let builder2 = thread::Builder::new();
        let t2 = builder2.spawn_unchecked(|| {
            println!("{}", a3);//0.5でprint
        });
        t1.unwrap().join();
        t2.unwrap().join();
    }

//         let immut_a_1 = ArcMut::to_immut(new_a);//immutableに、countは1
//         println!("{}", ArcImmut::strong_count(&immut_a_1));
//         let immut_a_2 = ArcImmut::clone_immut(&immut_a_1);//countは2に
//         println!("{}, {}", ArcImmut::strong_count(&immut_a_1), ArcImmut::strong_count(&immut_a_2));
