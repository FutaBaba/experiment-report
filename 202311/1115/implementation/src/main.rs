...
impl<T: ?Sized> BabaArcImmut<T> {
    pub fn clone_immut(&self) -> BabaArcImmut<T> {
        let old_size = unsafe { self.ptr.as_ref().strong.fetch_add(1, Relaxed)};
        if old_size > MAX_REFCOUNT {
            abort();
        }
        unsafe { BabaArcImmut { ptr: self.ptr } }
    }
    ...
    pub fn back_to_mut(this: Self) -> BabaArcMut<T> {
        ...
        unsafe { BabaArcMut { ptr: this.ptr } }
    }
}
...
impl<T> BabaArcMut<T> {
    pub fn new(data: T) -> BabaArcMut<T> {
        ...
        unsafe { BabaArcMut{ptr:Box::leak(x).into()} }
    }
}
...
fn main() {
    ...
    unsafe {
        ...
        let _t2 = builder2.spawn_unchecked(|| {
            ...
            let mut _mut_a = BabaArcImmut::back_to_mut(immut_a);
        }).unwrap().join();
   }
