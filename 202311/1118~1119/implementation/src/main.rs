...
fn main() {
    ...
    unsafe {
        let mut new_a = BabaArcMut::new(a);//1
        println!("{}", BabaArcMut::strong_count(&new_a));
        let immut_a1 = BabaArcMut::clone_immut(new_a);//immutableに、countは1
        println!("{}", BabaArcImmut::strong_count(&immut_a1));
        let immut_a2 = BabaArcImmut::clone_immut(&immut_a1);//countは2に
        println!("{}, {}", BabaArcImmut::strong_count(&immut_a1), BabaArcImmut::strong_count(&immut_a2));
