...
fn main() {
    let mut vec1: Vec<RefImmut<String>> = Vec::new();
    let mut vec2: Vec<RefImmut<String>> = Vec::new();
    let b1 = Builder::new();
    let b2 = Builder::new();
    ...
    vec2.push(s2);
    unsafe {
        let t1 = b1.spawn_unchecked(|| {
            thread::sleep(Duration::from_secs(1));
            println!("{}, from t1", vec1[0]);
            drop(vec1);
        });
        let t2 = b2.spawn_unchecked(|| {
            // thread::sleep(Duration::from_secs(1));
            println!("{}, from t2", vec2[0]);
            drop(vec2);
        });
        thread::sleep(Duration::from_secs(1));
        let mut mut_s = ss2.back_to_mut();
        mut_s.push_str("ssss");
        println!("{}, from main", mut_s);
        t1.unwrap().join();
        t2.unwrap().join();
    }
