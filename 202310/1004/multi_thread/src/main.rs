...
impl<T: ?Sized> BabaArc<T> {
    ...
    fn clone_mut(&self) -> BabaArc<T> {
        if Self::write_count(&self) == 1 && Self::read_count(&self) == 1 {
...
impl<T> BabaArc<T> {
    pub fn new(data: T) -> BabaArc<T> {
        ...
        let x: Box<_> = Box::new(BabaArcInner {
            ...
            write: atomic::AtomicUsize::new(1),
            read: atomic::AtomicUsize::new(1),
            data,
        });
...
fn g (a: &BabaArc<String>) -> Result<JoinHandle<()>, std::io::Error>{
    println!("{}, {}, from g", BabaArc::write_count(a), BabaArc::read_count(a));
